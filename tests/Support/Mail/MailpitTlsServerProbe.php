...
class MailpitTlsServerProbe
{
    ...
    public static function unavailableReason(): ?string
    {
        ...
        if ($host === '') {
            return self::$reason = 'TUXXEDO_TEST_MAILPIT_TLS_HOST is not set';
        }
        ...
        $apiReason = self::probeApi(
            apiUrl: MailpitTlsTestEnv::apiUrl(),
        );
        ...
        if ($caFile === '' || !\is_file($caFile)) {
            return self::$reason = \sprintf(
                'Mailpit-tls CA file not found (expected at %s)',
                $caFile === ''
                    ? '(unset)'
                    : $caFile,
            );
    ...
    private static function probeSmtp(
        ...
        return \sprintf(
            'Mailpit-tls SMTP probe on %s:%d failed after %d attempts: %s',
            $host,
            $port,
    ...
    private static function probeApi(
        ...
        if ($response === false) {
            return \sprintf(
                'Mailpit-tls API probe on %s failed (endpoint unreachable)',
                $apiUrl,
            );
        }
